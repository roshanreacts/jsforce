 ...
 * @param {String} [options.PKChunking] - To enable PK Chunking pass params.
...
Job.prototype.open = function(callback) {
  ...
  if (!this._jobInfo) {
    ...
    if(this.options.PKChunking && this.options.PKChunking != null) {
      headers["Sforce-Enable-PKChunking"] = this.options.PKChunking;
    };
  ...
  var poll = function() {
    ...
    self.check(function(err, res) {
      if (err) {
        ...
        } else if (res.state === "Completed" || res.state === "NotProcessed") {
          self.retrieve();
        } else {
